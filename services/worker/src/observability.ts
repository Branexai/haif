...
export function getWorkerMetrics() {
  const meter = metrics.getMeter(serviceName)
  const inferRequestsTotal = meter.createCounter('infer_requests_total', { description: 'Total inference requests' })
  const inferFailuresTotal = meter.createCounter('infer_failures_total', { description: 'Total inference failures' })
  const inferDurationSeconds = meter.createHistogram('infer_duration_seconds', { description: 'Inference generation duration', unit: 's' })
  return { inferRequestsTotal, inferFailuresTotal, inferDurationSeconds }
}